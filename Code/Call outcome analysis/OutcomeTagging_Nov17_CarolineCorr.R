...
summary |>
  mutate(
    outcome = case_when(
      ...
      last_activity == 'LegalServerScreenPop' & !is.na(last_agent) & time_gap > 0 ~ 'Positive',
      last_activity == 'LegalServerScreenPop' & is.na(last_agent) ~ 'Negative',
      ...
      last_activity == 'ScreenPopProcessComplete' & !is.na(last_agent) & time_gap > 0 ~ 'Positive',
      last_activity == 'ScreenPopProcessComplete' & is.na(last_agent) ~ 'Negative',
      # Hailee
  ...
  mutate(
    sub_type = case_when(
      ...
      last_activity == 'ScreenPopProcessComplete' & !is.na(last_agent) & time_gap > 0 ~ 'Agent Reached',
      last_activity == 'ScreenPopProcessComplete' & is.na(last_agent) ~ 'Abandoned',
      # Hailee
      #last_activity == 'AddressFaxHoursMenu' ~ 'Positive',
